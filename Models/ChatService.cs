...
public class ChatService
{
    ...
    public ChatService(ApplicationDbContext applicationDb)
    {
        teams = applicationDb.chatTeams.ToList();
        ...
        seniorityMultipliers =
            new Dictionary<UserRole, double> {
                { UserRole.Junior, 0.4 },
    ...
    public string CreateChatSession()
    {
        ...
        if (
            fifoQueue.Sessions.Count + overflowQueue.Sessions.Count >=
            maxQueueLength
        )
        {
    ...
    private UserRole
    DetermineNextAgentRole(
        Dictionary<UserRole, int> lastAssignedIndex,
        UserRole currentRole
    )
    {
        // Define the order of seniority (from lowest to highest)
        List<UserRole> seniorityOrder =
            new List<UserRole> {
                UserRole.Junior,
                UserRole.MidLevel,
                UserRole.Senior,
        ...
        for (int i = 1; i < seniorityOrder.Count; i++)
        {
            ...
            if (
                lastAssignedIndex[nextRole] <
                teams.FindAll(t => t.Agents.Contains(nextRole)).Count - 1
            )
            {
                return nextRole;
            }
    ...
    private void AssignChatsToAgents()
    {
        ...
        foreach (ChatSession session in fifoQueue.Sessions.ToList())
        {
            ...
            UserRole nextRoleToAssign =
                DetermineNextAgentRole(lastAssignedIndex,
            ...
            if (nextRoleToAssign != UserRole.Junior)
            {
                ...
                ChatTeam team =
                    teams.Find(t => t.Agents.Contains(nextRoleToAssign));

                if (team != null)
                {
                    // Assign the chat session to the next agent with the specified role
                    session.AssignedAgent = nextRoleToAssign;
                    lastAssignedIndex[nextRoleToAssign] =
                        (lastAssignedIndex[nextRoleToAssign] + 1) %
                        team.Agents.Count;
    ...
    private void StartChat(ChatSession session, UserRole assignedAgent)
    {
        ...
        Console
            .WriteLine($"Starting chat session {session.Id} with {
                assignedAgent}...");
        ...
        for (int i = 1; i <= 5; i++)
        {
            ...
            Console
                .WriteLine($"{assignedAgent}: Hi, this is agent {
                    assignedAgent}. This is agent response {i}");
        }

        // Simulate ending the chat session
        Console
            .WriteLine($"Chat session {session.Id} with {
                assignedAgent} ended.");
    ...
    private void PollChat(ChatSession session)
    {
        ...
        if (session.Status == ChatSessionStatus.Active)
        {
            ...
            Console
                .WriteLine($"Agent ({
                    session
                        .AssignedAgent}): We are here to help. Please wait...");
        ...
        else
        {
            ...
            Console
                .WriteLine("Chat session is inactive. You can start a new chat session if needed.");
        }
